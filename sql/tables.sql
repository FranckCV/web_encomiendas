...
CREATE TABLE empleado_salida (
  ...
  PRIMARY KEY (salidaid, empleadousuarioid)
);
...
CREATE TABLE escala (
  ...
  salidaid int(10) NOT NULL,
  PRIMARY KEY (sucursalid, salidaid)
);
...
CREATE TABLE seguimiento (
  ...
  paquetetracking int(11) NOT NULL,
  PRIMARY KEY (estado_encomiendaid, paquetetracking)
);
...
CREATE TABLE tarifa_ruta (
  ...
  PRIMARY KEY (sucursal_origen_id, sucursal_destino_id)
);
...
0 si es un empaque (paquete o sobre)
 ',
  ...
  PRIMARY KEY (num_serie, tipo_comprobanteid)
);
...
ALTER TABLE paquete
ADD CONSTRAINT FKpaquete395865 FOREIGN KEY (
    transaccion_encomienda_num_serie,
    transaccion_encomienda_tipo_comprobanteid
  ) REFERENCES transaccion_encomienda (num_serie, tipo_comprobanteid);
