...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const datosValidos = data.filter(envio => {
    return envio.destino && envio.destino.departamento && envio.destino.departamento.trim() !== '';
  });

  localStorage.setItem(STORAGE_KEY, JSON.stringify(datosValidos));
  console.log(`Registros válidos conservados: ${datosValidos.length}`);
});


// Agrega esto dentro de tu archivo registro_paquete.js, dentro de DOMContentLoaded
document.addEventListener("DOMContentLoaded", function () {
  const campoCodigo = document.getElementById('remitente-codigo');
  const campoConfirmacion = document.getElementById('remitente-cod_confirmacio');
  const mensajeCodigo = document.getElementById('mensaje-validacion-codigo');
  const mensajeConfirmacion = document.getElementById('mensaje-validacion-confirmacio');

  const solo4Digitos = /^\d{4}$/;

  // Validar campo principal (remitente-codigo)
  campoCodigo.addEventListener('input', () => {
    const valor = campoCodigo.value.trim();

    if (valor === '' || valor.length !== 4 || !solo4Digitos.test(valor)) {
      mensajeCodigo.textContent = 'Debe ingresar exactamente 4 números.';
      mensajeCodigo.style.display = 'block';
      campoCodigo.style.borderColor = '#fc8181';
    } else {
      mensajeCodigo.style.display = 'none';
      campoCodigo.style.borderColor = '#48bb78';
    }

    // Reinicia validación del segundo campo
    campoConfirmacion.style.borderColor = '';
  ...
  campoConfirmacion.addEventListener('input', () => {
    ...
    if (confirmacion === '' || confirmacion.length !== 4 || !solo4Digitos.test(confirmacion)) {
      mensajeConfirmacion.textContent = 'Debe ingresar exactamente 4 números.';
      mensajeConfirmacion.style.display = 'block';
      campoConfirmacion.style.borderColor = '#fc8181';
    } else if (codigo !== confirmacion) {
      mensajeConfirmacion.textContent = 'La confirmación no coincide con el código.';
      mensajeConfirmacion.style.display = 'block';
      campoConfirmacion.style.borderColor = '#fc8181';
    } else {
      mensajeConfirmacion.style.display = 'none';
      campoConfirmacion.style.borderColor = '#48bb78';
    }
  });
});
